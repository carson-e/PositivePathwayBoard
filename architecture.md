...
> 
> This architecture document was completely generated by AI based on code analysis and project structure. While it attempts to accurately describe the system architecture, design patterns, and implementation details, it may contain inaccuracies or become outdated as the project evolves.
> 
> This document is provided as a reference to help future developers understand the project structure and design decisions. Please verify critical details against the actual codebase and update this documentation as needed.
...
│  ┌──────────────────────────────────────────────────────┐   │
│  │  roster.db (SQLite)                                  │   │
│  │  - Student roster tables (Sheet1, Sheet2, etc.)      │   │
│  │  - Taps table (behavioral tracking)                  │   │
│  │  - Metadata table (teacher info)                     │   │
│  └──────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
```
...
The application uses a **single SQLite database** (`roster.db`) with multiple table types:

#### `roster.db` - Unified Database

**Purpose**: Store student roster data, behavioral tracking, and teacher metadata

**Table Types**:

##### 1. Roster Tables (Dynamic)
- Table names correspond to Excel sheet names (e.g., `Sheet1`, `Sheet2`)
- Columns dynamically created based on Excel structure
- Supports multi-sheet Excel files
- Repopulated on roster update (`if_exists='replace'`)
...
- Teacher metadata removed from roster sheets
- Stored in separate metadata table

##### 2. Taps Table - Behavioral Tracking

**Purpose**: Log all behavioral interactions (taps) with timestamps

**Schema**: Single table with indexes (within `roster.db`)
...
**Data Retention**: Permanent (no automatic deletion unless RESET button used)

##### 3. Metadata Table - Teacher Information

**Purpose**: Store teacher name and grade extracted from Excel header
...
INSERT INTO metadata VALUES ('Teacher Name', 'Ms. Johnson');
INSERT INTO metadata VALUES ('Grade', '5th Grade');
```

### Database Interaction Patterns

```python
# Read-Only Pattern (roster tables)
conn, cur = connect_db("roster.db")
cur.execute("SELECT * FROM Sheet1 WHERE [Student Name] = ?", (name,))
conn.close()

# Write-Heavy Pattern (taps table)
tracker = TapTracker()  # Manages connection lifecycle
tap_id = tracker.record_tap(student_name, tap_type, choice)

# Metadata access
cur.execute("SELECT value FROM metadata WHERE key = ?", ("Teacher Name",))
```
...
##### `GET /reports/town-hall-list`
**Purpose**: Get list of students eligible for town hall (≥ threshold % positive taps)

**Query Parameters**:
- `threshold` (optional, default: 95): Minimum positive percentage
- `month` (optional): Filter by month name
- `year` (optional): Filter by year

**Response**: CSV file with columns: Student Name, Positive Taps, Negative Taps, Positive Percentage

##### `GET /reports/student-report`
**Purpose**: Download student tap data as CSV

**Query Parameters**:
- `student` (required): Student name
- `month` (optional): Filter by month
- `year` (optional): Filter by year
- `format` (optional, default: "csv"): Output format

**Response**: CSV file with tap history
...
│       │       ├── ClassTableDropdown
│       │       ├── UpdateRosterButton
│       │       ├── PopulateRosterButton (RESET)
│       │       └── GenerateReportButton
│       ├── ReportDialog (conditional)
│       │   └── StudentListScroll
│       ├── TownHallPopup (conditional)
│       │   ├── StudentsList (eligible students)
│       │   └── DownloadCSVButton
│       └── MainContent
│           ├── LeftSection
│           │   ├── ChoiceTapsSection
...
│           │       ├── CurrentMonth (3 trait buttons)
│           │       ├── PreviousMonthCarousel
│           │       └── VideoLinkButton (optional)
│           └── RightSection (TriangleContainer)
│               ├── Row 0: SelectAllButton, TownHallButton
│               ├── Row 1: TeacherName, ClassGrade
│               └── Rows 2-8: StudentCards (33 total)
```
...
const [showTownHallPopup, setShowTownHallPopup] = useState(false);
const [showPreviousTraits, setShowPreviousTraits] = useState(false);
...
const [townHallStudents, setTownHallStudents] = useState<Student[]>([]);
...
const [badgeImage, setBadgeImage] = useState<number | null>(null);
...
type Student = {
  ...
  // Monthly behavior statistics (for town hall eligibility)
  monthlyPositiveTaps?: number;   // Count of positive taps this month
  monthlyNegativeTaps?: number;   // Count of negative taps this month
  monthlyPositivePercent?: number; // Percentage of positive taps
};
```
...
type MonthlyEquation = {
  ...
  result: string;
  color: string;
  badge_image: number;  // require() image asset
  videoUrl?: string;    // Optional YouTube/video link
};

const monthlyEquations: Record<number, MonthlyEquation> = {
  0:  { trait1: '+ Goodness',      trait2: '+ Skills',       result: 'Ability', color: '#ffcdd2', badge_image: require('./assets/...') },
  1:  { trait1: '+ Hope',          trait2: '+ Knowledge',    result: 'Discernment', color: '#fff9c4', badge_image: require('./assets/...') },
  2:  { trait1: '+ Respect',       trait2: '+ Gentleness',   result: 'Friendships', color: '#d9e2f7', badge_image: require('./assets/...') },
  3:  { trait1: '+ Self-Control',  trait2: '+ Purpose',      result: 'Resilience', color: '#ffcdd2', badge_image: require('./assets/...') },
  4:  { trait1: '',                trait2: '',               result: '', color: '#fff9c4', badge_image: null }, // Summer
  5:  { trait1: '',                trait2: '',               result: '', color: '#d9e2f7', badge_image: null },
  6:  { trait1: '',                trait2: '',               result: '', color: '#ffcdd2', badge_image: null },
  7:  { trait1: '+ Helpfulness',   trait2: '+ Organization', result: 'Learning Environment', color: '#fff9c4', badge_image: require('./assets/...') },
  8:  { trait1: '+ Care',          trait2: '+ Safety',       result: 'Stability', color: '#d9e2f7', badge_image: require('./assets/...') },
  9:  { trait1: '+ Joy',           trait2: '+ Focus',        result: 'Learning Energy', color: '#ffcdd2', badge_image: require('./assets/...') },
  10: { trait1: '+ Patience',      trait2: '+ Excellent Senses', result: 'Perception', color: '#fff9c4', badge_image: require('./assets/...') },
  11: { trait1: '+ Kindness',      trait2: '+ Understanding', result: 'Responsibility', color: '#d9e2f7', badge_image: require('./assets/...') },
};
```
...
| File | Lines | Purpose |
|------|-------|---------|  
| `App.tsx` | 1,768 | Entire frontend UI |
| `server.py` | 426 | API routing |
| `tap_tracker.py` | 296 | Behavior CRUD |
| `generate_reports.py` | ~300 | PDF generation |
| `PPB_DB.py` | 273 | Excel parsing |
| **Total Code** | **~3,063** | Core application logic |---
...
**`roster.db`** (Single database with multiple table types):

**Roster Tables** (Sheet1, Sheet2, etc.):
```
┌─────────────────────────────────┐
│           Sheet1                │
├─────────────────────────────────┤
...
# Check database (all tables in roster.db)
sqlite3 roster.db "SELECT * FROM Sheet1 LIMIT 5;"
sqlite3 roster.db "SELECT * FROM taps ORDER BY timestamp DESC LIMIT 10;"
sqlite3 roster.db "SELECT * FROM metadata;"

# List all tables
sqlite3 roster.db ".tables"
